...
2. 一些概念：
   ...
   子树、根、度、高度与深度(分为**树**的高度与深度和**某个结点**的高度与深度)

   ![img](https://wx4.sinaimg.cn/mw690/005LasY6ly1gquogtk1zdj30vq0fpwgs.jpg)

   ![img](https://wx4.sinaimg.cn/mw690/005LasY6ly1gquoi72bdbj30vq0fugmd.jpg)
...
3. 性质：
   ...
   **逻辑上**：
   ...
   + 第i层最多有$2^{i-1}$个结点。
   + 深度为k的二叉树最多有$2^k - 1$个结点。(等比数列求和，此时为完全二叉树)
   + 任意一棵二叉树，$n_0 = n_2 + 1$， 其中n0是度为0的结点，n2是度为2的结点。
...
4. 存储方式
   ...
   + 链式存储：
     ...
     比较常用，大部分二叉树都是通过这种结构来实现的。但是==内存消耗较大==

     ![img](https://wx3.sinaimg.cn/mw690/005LasY6ly1gquops9xd8j30vq0kemyi.jpg)
   ...
   + 数组存储：
     ...
     ![img](https://wx3.sinaimg.cn/mw690/005LasY6ly1gquoppeibyj30vq0gsjsd.jpg)
...
![img](https://wx1.sinaimg.cn/mw690/005LasY6ly1gquogxdvd9j30cn0eftbc.jpg)
...
1. 性质：
   ...
   3. 包含n个节点的完全二叉树二叉树高度为$logn$ (==证明？？==)
   
      > 假设高度为h，那么$2^{h-1}-1 < n <= 2^{h}-1$
      >
      > 所以$2^{h-1} <= n < 2^{h}$
      >
      > 所以$h-1 <= logn < h$
